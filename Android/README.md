...
### 스트림이 없는 캠페인에서 관측한 것 (기록)

당시 데모 캠페인 `faea28dd96c3` 은 플레이어가 검게만 나왔습니다. 앱·에뮬레이터 문제가 아니라
**스트림이 없었습니다**.
...
> **✅ 이후 정정 (2026-07-30 22:09):** `faea28dd96c3` 은 그 뒤 **재설정되어 현재 정상 재생됩니다.**
> iOS 에서 실측한 결과 404 는 0건이고(200 8건 · 206 56건) 재생 헤드가 10초→80초로 계속 진행했습니다.
> 다만 스트림이 **Apple 의 공개 샘플 HLS(BipBop, 1920x1080@60fps VOD)** 로 연결돼 있어,
> 채팅·상품·LIVE 배지처럼 실제 방송 중에만 나타나는 요소는 이 캠페인으로 확인할 수 없습니다.
>
> 위 404 기록은 **당시 관측을 그대로 보존한 것**입니다 — 아래 "SDK 팀에 보고할 관측 3건" 이
> 이 상황에서 나온 것이므로 함께 읽어야 맥락이 맞습니다. 그 3건은 아직 재확인되지 않았습니다.
